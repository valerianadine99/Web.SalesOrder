...
import { Order, UpdateOrderCommand } from '@/types/api';
...
const EditOrderModal: React.FC<EditOrderModalProps> = ({ isOpen, order, onClose }) => {
  ...
  useEffect(() => {
    if (order && isOpen) {
      ...
      setValue('orderDate', order.orderDate.split('T')[0]);
      
      // Convertir OrderDetail a CreateOrderDetailDto
  ...
  const onSubmit = async (data: UpdateOrderFormData) => {
    ...
    try {
      const updateData: UpdateOrderCommand = {
        id: order.id,
        ...data,
      };
      
      await updateOrder(order.id, updateData);
      toast('Orden actualizada exitosamente', { icon: '✅' });
      onClose();
    } catch (error) {
      toast('Error al actualizar la orden', { icon: '❌' });
    } finally {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <Button
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Actualizando Orden...' : 'Actualizar Orden'}
            </Button>
          </div>
        </form>
      </div>
